...
import { addItem, removeItem } from '../redux/cartSlice'; // Ajusta la ruta según tu store

function ProductList({ onHomeClick }) {
  const dispatch = useDispatch();
  const cartItems = useSelector(state => state.cart.items); // Redux state
  const [showCart, setShowCart] = useState(false);
  const [showPlants, setShowPlants] = useState(false);
  const [addedToCart, setAddedToCart] = useState({});

  // Actualiza los botones "Added" según el estado del carrito
  useEffect(() => {
    const addedState = {};
 ...
 const plantsArray = [
  ...
  const handleRemoveFromCart = plantName => {
    ...
    setAddedToCart(prev => ({ ...prev, [plantName]: false }));
  };
  ...
  return (
    <div>
      ...
      {showCart ? (
        <CartItem cartItems={cartItems} onRemove={handleRemoveFromCart} onContinueShopping={handleContinueShopping} />
      ) : showPlants ? (
